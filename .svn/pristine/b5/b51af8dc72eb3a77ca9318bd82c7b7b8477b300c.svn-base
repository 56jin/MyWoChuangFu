...
- (void)scrollViewDidEndDragging:(UIScrollView *)sender willDecelerate:(BOOL)decelerate{
    //判断是否拖拽到头或者尾
    float xOffset = sender.contentOffset.x;
    float conWidth = sender.contentSize.width;
    float rightOffset = xOffset - conWidth + sender.bounds.size.width;
    
    if (xOffset<-40.0) {
        if (_delegate && [_delegate respondsToSelector:@selector(paginatorView:dragginToLeft:)]) {
            [_delegate paginatorView:self dragginToLeft:self.currentPageIndex];
        }
    ...
    if (rightOffset > 40.0) {
        if (_delegate && [_delegate respondsToSelector:@selector(paginatorView:dragginToRight:)]) {
            [_delegate paginatorView:self dragginToRight:self.currentPageIndex];
        }
    }
