...
@interface ChoosePackageVC ()<SCNavTabBarControllerDelegate,HttpBackDelegate,SCNavTabBarDelegate,TitleBarDelegate,UIAlertViewDelegate>
{
...
@property (nonatomic,strong)NSString *totalPrice; //合约机总价

@property (nonatomic,strong)ZSYCommonPickerView *packagePicker;
@property (nonatomic,strong)ZSYCommonPickerView *timePicker;
@end
...
- (void)viewDidLoad {
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                 self.attributes[@"skuId"],@"skuId",
                                 self.attributes[@"productId"]==nil?[NSNull null]:self.attributes[@"productId"],@"productId",
                                 self.attributes[@"moduleId"]==nil?[NSNull null]:self.attributes[@"moduleId"],@"moduleId",
                                 @"",@"lowCost",
                                 @"",@"pkgId",nil];
    self.SendDic = dict;
    [bus package:dict];
    
    //     self.attributes[@"skuId"]
    ...
    lable00.textAlignment = NSTextAlignmentRight;
    ...
    lable80.textAlignment = NSTextAlignmentRight;
...
-(void)layoutView
{
    ...
    _scrollview = [[UIScrollView alloc]initWithFrame:CGRectMake(0,103, SCREEN_WIDTH, SCREEN_HEIGHT-20)];
    ...
    UIView *view3 = [[UIView alloc]initWithFrame:CGRectMake(0 ,64, SCREEN_WIDTH, 40)];
    ...
    lable00.frame = CGRectMake(SCREEN_WIDTH/2, 48-10, 120, 44);
    ...
    lable80.frame = CGRectMake(SCREEN_WIDTH/2, 308-33, 120, 44);
...
- (void)itemDidSelectedWithIndex:(NSInteger)index
{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                 self.attributes[@"skuId"],@"skuId",
                                 self.attributes[@"productId"],@"productId",
                                 self.attributes[@"moduleId"],@"moduleId",
                                 _lowCost,@"lowCost",
                                 @"",@"pkgId",nil];
    [bus package:dict];
    ...
    if([[ChoosePackageMessage getBizCode] isEqualToString:bizCode]){
        if([oKCode isEqualToString:errCode]){
            ...
            for (NSDictionary *dict in _packageTypes)
            {
                ...
                [PackManager shareInstance].packageModes = [_defautData objectForKey:@"packageModes"];
            }
...
-(void)choosePackClicked:(UIButton *)sender
{
    ...
    ZSYCommonPickerView *pickerView = [[ZSYCommonPickerView alloc] initWithTitle:title
                                                               ...
                                                               selectedIndexPath:0
                                                                        Firstrow:@"" cancelButtonBlock:^{
                                                                            ...
                                                                            [PackManager shareInstance].packageModes = [ dict1 objectForKey:@"packageModes"];
...
#pragma mark 选择期限
-(void)chooseTimeClicked:(UIButton *)sender
{
    sender.userInteractionEnabled = NO;
                                                               ...
                                                               selectedIndexPath:0
                                                                        Firstrow:@"" cancelButtonBlock:^{
                                                                            ...
                                                                            if (_timeStyle==nil){
                                                                                ...
                                                                                [lable80 setTextColor:[UIColor blueColor]];
...
- (void)saveClicked{
    ...
    if (_packTypeName==nil&&_timeStyle!=nil) {
        ...
        [self ShowProgressHUDwithMessage:@"请选择期限！"];
        
    }else if(_packTypeName==nil&&_timeStyle==nil){
        
        [self ShowProgressHUDwithMessage:@"请选择套餐期限！"];
    }else {
        [self ShowProgressHUDwithMessage:@"保存成功"];
        [self backAction];
        //        NSString *str = [NSString stringWithFormat:@"%@/%@/%@个月", _packTypeName,_timeStyle,_packId];
        if (self.block)
        {
            NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:_lowCost,@"_lowCost", _timeStyle,@"period",_packTypeName,@"packTypeName",_packId,@"pkgId",_totalPrice,@"totalPrice", nil];
            self.block(dict);
        }
