...
@property (nonatomic, strong) NSMutableArray *numPriceArray;
...
-(void)reloadNumList
{
    ...
    _requestDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                    ...
                    @"",@"numType",
                    _productID == nil?@"":_productID,@"productId",
                    @"1",@"pageIndex",
                    @"20",@"pageCount",
                    @"initSearch",@"requestType",nil];
    [bus number:_requestDict];
    [_dataArray removeAllObjects];
    [_numPriceArray removeAllObjects];
...
-(void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
        [NumberManager shareInstance].num = nil;
    _areaName =[[NSUserDefaults standardUserDefaults] objectForKey:@"currentCity"];
    if (_areaName==nil) {
        _areaName = @"";
    }
...
- (void)viewDidLoad {
    ...
    _numPriceArray = [NSMutableArray array];
...
-(void)requestDidFinished:(NSDictionary *)info
{
    ...
    for (NSDictionary *dict in phoneNums)
    {
        ...
        [_numPriceArray addObject:dict[@"phoneNumPrice"]];
    }
...
//    _countLable.text = [NSString stringWithFormat:@"总共%d件商品",_dataArray.count];
    ...
    if (currentCity != nil)
    {
        if (_ifFirst ==YES)
        {
            ...
            {
                NSRange range =[currentCity rangeOfString:dict[@"areaName"]];
                if (range.location == NSNotFound ||range.length ==0)
                {
                    range = [dict[@"areaName"] rangeOfString:currentCity];
                    if (range.location!= NSNotFound &&range.length>0)
                    {
                        _areaCode = dict[@"areaCode"];
                        [_btnArea setTitle:dict[@"areaName"] forState:UIControlStateNormal];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:[_dataArray objectAtIndex:indexPath.row],@"phoneNum",[_numPriceArray objectAtIndex:indexPath.row],@"phoneNumPrice", nil];
    
    [NumberManager shareInstance].num = item;
    
    if (self.block)
    {
//        self.block([_dataArray objectAtIndex:indexPath.row]);
        self.block(dict);
    }
    // 刷新界面
    [_tableView reloadData];
