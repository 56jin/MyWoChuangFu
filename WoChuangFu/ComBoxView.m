...
//    btn.layer.borderWidth = 0.5;
    ...
    titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width-imgW-7, self.frame.size.height)];
    titleLabel.font = [UIFont systemFontOfSize:16];
    titleLabel.textColor = [ComponentsFactory createColorByHex:@"#666666"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell==nil)
    {
        ...
        label.textColor = [ComponentsFactory createColorByHex:@"#666666"];
...
//        label.textColor = kTextColor;
    ...
    label.text = [_titlesList objectAtIndex:indexPath.row][@"areaName"];
...
{
    NSDictionary *dict = _titlesList[indexPath.row];
    titleLabel.text = dict[@"areaName"];
    _isOpen = YES;
    [self tapAction];
    if([_delegate respondsToSelector:@selector(comBoxView:didSelectAtIndex:withData:)])
    {
        [_delegate comBoxView:self didSelectAtIndex:indexPath.row withData:dict];
    }
