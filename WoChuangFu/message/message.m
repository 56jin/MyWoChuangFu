...
- (void)parse:(NSString *)responseMessage {
    ...
    MyLog(@"responseMessage=%@",responseMessage);
#ifdef STATIC_XML
...
-(NSString *)getRspcode
{
	...
	if ([json_respon_header count] == 0) {
        ...
        NSString* identityId = [json_respon_header objectForKey:@"identityId"];
        if(identityId != nil && (NSObject*)identityId != [NSNull null] && identityId.length > 0){
            [[NSUserDefaults standardUserDefaults] setObject:identityId forKey:@"sessionId"];
